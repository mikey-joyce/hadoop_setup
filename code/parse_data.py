...
def main():
    ...
    sentiment_counts = valid_none['sentiment'].value_counts()
    sentiment_counts_pd = sentiment_counts.to_pandas()

    plt.figure(figsize=(6, 6))
    sentiment_counts_pd.plot.pie(autopct='%1.1f%%', startangle=90)
    plt.title('Valid_none Sentiment Distribution')
    plt.ylabel('')
    plt.savefig(save_dir + 'valid_none_senti.png')
    plt.close()
    ...
    plt.ylabel('')
    plt.savefig(save_dir + 'test_senti.png')
    plt.close()
